...
entity RSACoreTestBenchcopy is
end RSACoreTestBenchcopy;

architecture struct of RSACoreTestBenchcopy is
...
begin
  ...
  CryptoCtrl : process(Clk, Resetn)
    ...
    variable s128 : string(4*W_BLOCK/4 downto 1);
  begin
    if (Resetn = '0') then
      ...
      case CryptoState is
        ...
        when e_READ_DATA =>
          ...
          -- YXNEKey (Y128 bit + X128bit + N128 bit + E128 bit)
          readline(ComFile, l);
          read(l, s128);
          NEKey <= str_to_stdvec(s128);
